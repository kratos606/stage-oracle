...
function Plan(props) {
  const [selected,setSelected]=useState(props.plan)
    ...
    const handleUpdate = async() => {
        await axios.put(`${BaseURL}/plan/${selected.id}`,{code_rlp:selected.code_rlp,ordre_jour:selected.ordre_jour,ordre_lecture_paquet:selected.ordre_lecture_paquet,tournée_debut:selected.tournée_debut,tournée_fin:selected.tournée_fin}).then(res => {     
            if(res.data.error) {
